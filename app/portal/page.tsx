...
import { getAnnouncements, getOrCreateProfile, getSchedule } from "@/lib/portal/queries";
...
export default async function PortalHomePage() {
	const [profile, announcements, schedule] = await Promise.all([
		getOrCreateProfile(),
		getAnnouncements(),
		getSchedule(),
	]);
			...
			>
				...
				<div className="flex w-full flex-wrap items-center justify-between gap-6 p-8">
					...
					<div className="flex items-center gap-4 rounded-sm bg-white py-2.5 pl-5 pr-2.5 shadow-pop">
						<CheckinCard checkedIn={profile?.checked_in ?? false} />
					</div>
				</div>
			</section>
