...
export function NfcTagList({ tags }: { tags: NfcTag[] }) {
	...
	const filtered = useMemo(() => {
		...
		return tags.filter(
			(tag) =>
				tag.fullName.toLowerCase().includes(needle) ||
				tag.email.toLowerCase().includes(needle),
		);
	}, [tags, query]);
	...
	return (
		<div className="flex flex-col gap-5">
			<input
				...
				className="h-12 w-full max-w-[420px] rounded-sm border border-black/10 bg-sun-50 px-4 font-body text-[14px] text-base-800 outline-none transition focus:border-sun-300 focus:bg-white"
			/>

			{filtered.length === 0 ? (
				<p className="font-body text-[14px] text-sun-400">
					{tags.length === 0
						? "No portal profiles yet."
						: `No hackers match “${query.trim()}”.`}
				</p>
			) : (
				<div className="overflow-hidden rounded-sm bg-surface-card shadow-card">
					{filtered.map((tag) => (
						<div
							key={tag.userId}
							className="flex flex-wrap items-center gap-4 border-t border-black/[0.06] px-5 py-4 first:border-t-0"
						>
							...
							<img
								...
								width={64}
								height={64}
								className="h-16 w-16 flex-shrink-0 rounded-sm bg-white"
							/>
